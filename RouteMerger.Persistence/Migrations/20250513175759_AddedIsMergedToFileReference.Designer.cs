// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using RouteMerger.Persistence.Data;

#nullable disable

namespace RouteMerger.Persistence.Migrations
{
    [DbContext(typeof(RouteMergerDbContext))]
    [Migration("20250513175759_AddedIsMergedToFileReference")]
    partial class AddedIsMergedToFileReference
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.4")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("RouteMerger.Persistence.Entities.FileReference", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTimeOffset?>("DeletedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("FileExtension")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<bool>("IsMerged")
                        .HasColumnType("boolean");

                    b.Property<DateTimeOffset>("LastUpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("RelativePath")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<Guid>("RouteId")
                        .HasColumnType("uuid");

                    b.Property<string>("UserProvidedName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("IsMerged");

                    b.HasIndex("RouteId");

                    b.ToTable("FileReferences");
                });

            modelBuilder.Entity("RouteMerger.Persistence.Entities.Route", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTimeOffset?>("DeletedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTimeOffset>("LastUpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid?>("MergedFileReferenceId")
                        .HasColumnType("uuid");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("Id");

                    b.HasIndex("MergedFileReferenceId")
                        .IsUnique();

                    b.ToTable("Routes");
                });

            modelBuilder.Entity("RouteMerger.Persistence.Entities.FileReference", b =>
                {
                    b.HasOne("RouteMerger.Persistence.Entities.Route", null)
                        .WithMany("FileReferences")
                        .HasForeignKey("RouteId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("RouteMerger.Persistence.Entities.Route", b =>
                {
                    b.HasOne("RouteMerger.Persistence.Entities.FileReference", "MergedFileReference")
                        .WithOne()
                        .HasForeignKey("RouteMerger.Persistence.Entities.Route", "MergedFileReferenceId");

                    b.Navigation("MergedFileReference");
                });

            modelBuilder.Entity("RouteMerger.Persistence.Entities.Route", b =>
                {
                    b.Navigation("FileReferences");
                });
#pragma warning restore 612, 618
        }
    }
}
